...
const Card = ({ props: {
  ...
  return (
    <section className="flex inline-block flex-col py-6 px-4 shadow-md col-span-1 rounded-2 hover:bg-surface-100 bg-white cursor-pointer">
      <div className="flex items-center">
        <img className="h-8 w-8 rounded mr-3" src={featured_image} alt=""/>
        <p className="font-duru text-surface-800">Php development</p>
      </div>
      <div className="flex flex-col px-6 mt-3">
        <img className="w-full h-60 object-cover rounded-2" src={featured_image} alt=""/>
        <section className="flex flex-col mt-3">
          <p className="font-exo text-xl font-bold text-surface-800 text-center">{title}</p>
